...
Consider a spam filter. It has one positive class and two kinds of legitimate email:

| Group | Example | Binary label |
|---|---|---:|
| \(P\) | A spam campaign | 1 |
| \(N_1\) | An ordinary personal or work message | 0 |
| \(N_2\) | A legitimate newsletter or promotional update that resembles spam | 0 |

An ordinary message may be easy to separate from spam. A newsletter is harder: it can contain marketing language, repeated links, tracking parameters and a sender the recipient has never replied to. Even so, both \(N_1\) and \(N_2\) must produce the same “not spam” result.

This is only a running analogy; the original domain and numerical results are omitted. The shape of the problem is the part that matters. A single binary classifier must learn an easy \(P\)-versus-\(N_1\) boundary and a much less obvious \(P\)-versus-\(N_2\) boundary.

I expected the harder negative group to need more attention. In my existing evaluation, giving it half as much weight improved both precision and recall on validation and held-out test data.

A specialist trained only on \(P\) and \(N_2\) did not provide enough evidence to justify a second model. That result mattered because I had started with an architecture question: should I split the problem across two models?

The experiment did not prove that cascades are a bad idea. I did not evaluate a complete routed system. It gave me a more useful default: before adding a model, test whether the current objective is giving each part of the data the right amount of influence.
...
\[
\mathcal{L}
= \sum_i a_{g_i}\operatorname{BCE}(y_i,p_i).
\]

The same experiment can begin with:

\[
a_P=1, \qquad a_{N_1}=1, \qquad a_{N_2}=\alpha.
\]
